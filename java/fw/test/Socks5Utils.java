...
class Socks5Utils {
    ...
    public static String createDigest(String sessionID, String initiatorJID, String targetJID) {
        ...
        return b.toString();//StringUtils.hash(b.toString());
    }
     ...
     */
    public static byte[] receiveSocks5Message(DataInputStream in,String addres) throws IOException {
        byte[] header = new byte[5];
        in.readFully(header, 0, 5);

        if (header[3] != (byte) 0x03 && header[3] != (byte) 0x01 ) {
        	System.out.println("Unsupported SOCKS5 address type: "+Integer.toHexString(header[3]));
        }

        switch (header[3]) {
		case 0x01:			
			System.out.println(Printer.printData(header));
			System.out.print("IP: ");
			//addres = header[4]+"."+header[5]+"."+header[6]+"."+header[7];
