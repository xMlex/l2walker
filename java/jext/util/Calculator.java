...
public class Calculator {
			...
			if (term) {
				if (c == '(') {
					...
					if (j < s.length()) {
						...
						if (c == '(') {
							FuncNode fn = _this.new FuncNode(s.substring(i, j),
									negate);
							int b = 1;
							int kOld = k + 1;
							while (b != 0) {
	...
	private static Expression build(Stack s1, Stack s2) {
		...
		while (!s4.isEmpty()) {
			...
			if (o.equals("*"))
				s3.addToTail(_this.new MultNode((Expression) o1,
						(Expression) o2));
			else if (o.equals("/"))
	...
	private class AddNode extends OpNode {
		...
		/**
		 * Adds the evaluation of the left side to the evaluation of the right
	...
	private class DivNode extends OpNode {
		...
		/**
		 * Divides the evaluation of the left side by the evaluation of the
		 * right side and returns the result.
		 */
		@Override
		public double eval(VarMap v, FuncMap f) {
			double a = leftChild.eval(v, f);
	...
	private class MultNode extends OpNode {
		...
		/**
		 * Multiples the evaluation of the left side and the evaluation of the
		 * right side and returns the result.
		 */
		@Override
		public double eval(VarMap v, FuncMap f) {
	...
	private abstract class Expression {
		...
		/**
		 * Returns the result of evaluating the expression tree rooted at this
		 * node.
		 */
		public abstract double eval(VarMap v, FuncMap f);

		/**
		 * Returns true if this node is a descendent of the specified node,
		 * false otherwise. By this methods definition, a node is a descendent
		 * of itself.
		 */
		...
		/**
		 * Returns a string that represents the expression tree rooted at this
		 * node.
		 */
		@Override
	...
	private class FuncNode extends TermNode {
		...
		/**
		 * Evaluates each of the children, storing the result in an internal
		 * double array. The FuncMap is used to obtain a Function object based
		 * on the name of this FuncNode. The function is passed the double array
		 * and returns a result. If negate is true, the result is negated. The
		 * result is then returned. The numParam passed to the function is the
		 * number of children of this FuncNode.
		 */
		@Override
		public double eval(VarMap v, FuncMap f) {
	...
	private abstract class TermNode extends Expression {
		...
		/**
		 * Returns true if the term should negate the result before returning it
		 * in the eval method.
		 */
		...
		/**
		 * Sets the name of the term. Valid names must not begin with a digit or
		 * a decimal, and must not contain round brackets, operators, commas or
		 * whitespace.
		 * 
		 * @throws IllegalArgumentException
	...
	private class PowNode extends OpNode {
		...
		/**
		 * Raises the evaluation of the left side to the power of the evaluation
		 * of the right side and returns the result.
		 */
		@Override
		public double eval(VarMap v, FuncMap f) {
			double a = leftChild.eval(v, f);
	...
	private class FuncMap {
		...
		/**
		 * Adds the mappings for many common functions. The names are specified
	...
	private interface Function {
		 ...
		 * Functions that accept a variable number of inputs can take numParam
		 * to be the number of inputs.
		 */
		public double of(double[] param, int numParam);

		/**
		 * Returns true if the numParam is an accurate representation of the
		 * number of inputs the function processes.
		 */
	...
	private class AcoshFunction implements Function {
		...
		/**
		 * Returns the value of 2 * ln(sqrt((x+1)/2) + sqrt((x-1)/2)), where x
		 * is the value at index location 0.
		 */
		public double of(double[] d, int numParam) {
	...
	private class CombinFunction implements Function {
		...
		/**
		 * Returns the number of ways r items can be chosen from n items. The
		 * value of n is (int) d[0] and the value of r is (int) d[1].
		 */
		public double of(double[] d, int numParam) {
			int n = (int) d[0];
			int r = (int) d[1];
	...
	private class CoshFunction implements Function {
		 ...
		 * (<i>e<sup>x</sup>&nbsp;+&nbsp;e<sup>-x</sup></i>)/2, where x is the
		 * value at index location 0 and <i>e</i> is the base of natural
		 * logarithms.
		 */
		public double of(double[] d, int numParam) {
	...
	private class ExpFunction implements Function {
		...
		/**
		 * Returns Euler's number, <i>e</i>, raised to the exponent of the value
		 * at index location 0.
		 */
		public double of(double[] d, int numParam) {
			return Math.exp(d[0]);
	...
	private static final class PascalsTriangle {
		...
		/**
		 * The nCr function returns the number of ways r things can be chosen
		 * from a set of size n. Mathematically, it is defined as: n! / (r! * (n
		 * - r)!) Although the result is always a whole number, double precision
		 * is used because the maximum value a double can represent is larger
		 * than long. Thus, large returned values will only be an approximation
		 * of the actual value. If the result exceeds the capabilities of double
		 * precision then the result can be checked using
		 * Double.isInfinite(...). For example:
		 * System.out.println(PascalsTriangle.nCr(1030, 515)); // outputs:
		 * Infinity If the value of n or r is less than 0 or the value of r is
		 * greater than n then 0 is returned.
		 */
	...
	private class LogFunction implements Function {
		...
		/**
		 * If the number of parameters specified is 1, then the log base 10 is
		 * taken of the value at index location 0. If the number of parameters
		 * specified is 2, then the base value is at index location 1.
		 */
		public double of(double[] d, int numParam) {
			if (numParam == 1)
	...
	private class MaxFunction implements Function {
		...
		/**
		 * Returns the maximum value of the specified inputs. Double.MAX_VALUE
		 * is returned for 0 parameters.
		 */
		public double of(double[] d, int numParam) {
			if (numParam == 0)
	...
	private class MinFunction implements Function {
		...
		/**
		 * Returns the minimum value of the specified inputs. Double.MIN_VALUE
		 * is returned for 0 parameters.
		 */
		public double of(double[] d, int numParam) {
			if (numParam == 0)
	...
	private class ModFunction implements Function {
		...
		/**
		 * Returns the value of x % y, where x = d[0] and y = d[1]. More
		 * precisely, the value returned is x minus the value of x / y, where x
		 * / y is rounded to the closest integer value towards 0.
		 */
		public double of(double[] d, int numParam) {
			return d[0] % d[1];
		}
	...
	private class RandFunction implements Function {
		...
		/**
		 * Returns a random value in the range [0, 1) that does not depend on
		 * the input.
		 */
		public double of(double[] d, int numParam) {
	...
	private class RoundFunction implements Function {
		...
		/**
		 * Returns the value at d[0] rounded to the nearest integer value. If
		 * the value exceeds the capabilities of long precision then the value
		 * itself is returned.
		 */
		public double of(double[] d, int numParam) {
	...
	private class TanhFunction implements Function {
		 ...
		 * /(<i>e<sup>x</sup>&nbsp;+&nbsp;e<sup>-x</sup></i>), where x is the
		 * value at index location 0 and <i>e</i> is the base of natural
		 * logarithms.
		 */
		public double of(double[] d, int numParam) {
