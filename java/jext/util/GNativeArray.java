...
public abstract class GNativeArray<E> extends GArray<E> {
	...
	public GNativeArray(int initialCapacity) {
		if (initialCapacity < 0)
			throw new IllegalArgumentException("Illegal Capacity: "
					+ initialCapacity);
		elementData = (E[]) Array.newInstance(
				_getTypeArguments(GNativeArray.class, getClass()).get(0),
				initialCapacity);
	}
	 ...
	 */
	public static <T> List<Class<?>> _getTypeArguments(Class<T> baseClass,
		...
		Type[] actualTypeArguments = type instanceof Class<?> ? ((Class<?>) type)
